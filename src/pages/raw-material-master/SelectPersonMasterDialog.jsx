...
const SelectPersonMasterDialog = ({ open, setPersonId, setOpen, personIdName, searchTerm,setSearchTerm }) => {
  ...
  const fetchCategories = async (currentPage) => {
    try {
      ...
      const response = await axiosInstance.get(`/personMaster?page=${currentPage}&records_per_page=${recordsPerPage}&search=${searchValue}`);
          ...
          <List>
            {personName.map((person, index) => (
              <ListItem
                ...
                onClick={() => {
                  setPersonId({ id: person.category_master_id, uuid: person.uuid, name: person.name });
                  setOpen(false);
                }}
              >
                {person.name}
