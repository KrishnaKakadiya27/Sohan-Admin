...
const DeleteMachineMaster = ({id,getListData}) => {
    ...
    const handleDelete = async () => {
        try {
            const response = await axiosInstance.delete(`machineMaster?uuid=${id}`)
            if (response?.status === 200) {
              toast.success("Deleted machine data successfully")
                getListData();
                handleClose();
            }
        } catch (error) {
  ...
  return (
    ...
    >
        <DialogTitle id="alert-dialog-title">{"Delete Machine Master"}</DialogTitle>
        <DialogContent>
